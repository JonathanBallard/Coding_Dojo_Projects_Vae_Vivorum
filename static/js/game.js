...
if(gameReady == false){
    document.onkeydown = function(e) {
        ...
        if(e.keyCode == 32){
            console.log('gameReady True, keycode ' + e.keyCode)
            gameReady = true;
...
function gameStartDelay(){
    setTimeout(startGame, preGamePause)
}
...
function startGame(){
    ...
    // first remove countdown from screen
    $('#startCountdown').text('');
    $('#countdownNum').text('');
    ...
    function startingEnemies(){
        ...
        enemies.push(startingEnemy1)
        enemies.push(startingEnemy2)
        enemies.push(startingEnemy3)
        enemies.push(startingEnemy4)
    }
    ...
    // Once player has pressed any button to start game



    //Play Game Loop - Original Location
